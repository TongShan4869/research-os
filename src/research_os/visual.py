  ...
  <script>
    ...
    document.querySelectorAll(".mode-toggle [data-view]").forEach(function (button) {{
      button.addEventListener("click", function () {{
        state.view = button.getAttribute("data-view") || "project";
        applyView();
        draw();
      }});
    }});
    ...
    function curvePath(source, target) {{
      ...
      const curve = Math.min(80, Math.max(24, Math.sqrt(dx * dx + dy * dy) * 0.12));
      const controlX = midX - dy / Math.max(1, Math.sqrt(dx * dx + dy * dy)) * curve;
      const controlY = midY + dx / Math.max(1, Math.sqrt(dx * dx + dy * dy)) * curve;
